...
export function useInvoicePreview(params: InvoiceParams) {
  return useQuery({
    ...
    enabled: !!(params.importId && params.serviceProviderId && params.paymentDate && params.paidOnDate),
  })
}
...
export function useDownloadInvoicePdf() {
  return useMutation({
    ...
    onSuccess: (blob, params) => triggerDownload(blob, `provider-invoice-${params.serviceProviderId}.pdf`),
  })
}
...
export function useDownloadInvoiceCsv() {
  return useMutation({
    ...
    onSuccess: (blob, params) => triggerDownload(blob, `provider-invoice-${params.serviceProviderId}.csv`),
  })
}
