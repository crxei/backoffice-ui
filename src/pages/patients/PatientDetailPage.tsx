...
import { ArrowLeft, Phone, Mail, MapPin, CheckCircle, Clock, Users } from 'lucide-react'
...
import { patientJourneys, type ContinuumStage } from '../../data/patientJourneys'
import { careGaps } from '../../data/careGaps'
import { careProtocols, frequencyLabel, teamLabel } from '../../data/careProtocols'
...
const tabs = ['Overview', 'Clinical Data', 'Appointments', 'Care Plans', 'Communications', 'Journey']
...
const stageOrder: Record<ContinuumStage, number> = {
  intake: 0, enrolled: 1, assessment: 2, active_care: 3, maintenance: 4, discharged: 5,
}
...
const adherenceLabel = (score: number) =>
  score >= 80 ? 'Good' : score >= 60 ? 'Moderate' : score >= 40 ? 'Poor' : 'Critical'
...
export function PatientDetailPage() {
  ...
  const patientGaps = careGaps.filter((g) => g.patientId === id && g.status !== 'resolved')
                  ...
                  {STAGES.map((stage, i) => {
                    ...
                    return (
                      <div key={stage.key} className="flex items-center flex-shrink-0">
                        <div className={`flex flex-col items-center px-3 py-2 rounded-lg min-w-[90px] ${isActive ? 'bg-blue-600 text-white' : isDone ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-400'}`}>
                          <div className={`h-5 w-5 rounded-full border-2 mb-1 flex items-center justify-center ${isActive ? 'border-white bg-white' : isDone ? 'border-green-500 bg-green-500' : 'border-gray-300 bg-white'}`}>
                            {isDone && <CheckCircle className="h-3.5 w-3.5 text-white" />}
                            {isActive && <div className="h-2 w-2 rounded-full bg-blue-600" />}
                          </div>
                          <span className={`text-xs font-medium text-center leading-tight ${isFuture ? 'text-gray-400' : ''}`}>{stage.label}</span>
                        </div>
                        {i < STAGES.length - 1 && (
                          <div className={`h-0.5 w-6 flex-shrink-0 ${stageOrder[stage.key] < current ? 'bg-green-400' : 'bg-gray-200'}`} />
                        )}
                      </div>
                    )
                  })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-sm font-bold ${journey.adherenceScore >= 80 ? 'text-green-600' : journey.adherenceScore >= 60 ? 'text-yellow-600' : journey.adherenceScore >= 40 ? 'text-orange-600' : 'text-red-600'}`}>
                        {journey.adherenceScore}% — {adherenceLabel(journey.adherenceScore)}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div className={`h-2.5 rounded-full ${adherenceColor(journey.adherenceScore)}`} style={{ width: `${journey.adherenceScore}%` }} />
                    </div>
                    <p className="text-xs text-gray-400 mt-1">
                      {journey.completedActivitiesLast30} of {journey.totalActivitiesLast30} activities completed in the last 30 days
                    </p>
                  </div>
                  <div className="pt-2 border-t border-gray-100">
                    <p className="text-xs text-gray-500 mb-1 font-medium uppercase tracking-wide">Next Action</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2 border-t border-gray-100 text-xs">
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{format(parseISO(journey.lastContactDate), 'MMM d, yyyy')}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Next Contact</p>
                      <p className="font-medium text-gray-900">{format(parseISO(journey.nextContactDate), 'MMM d, yyyy')}</p>
                    </div>
                    <div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                  ...
                  {patientGaps.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {patientGaps.map((gap) => (
                        <div key={gap.id} className={`p-3 rounded-lg border ${gap.status === 'critical' ? 'bg-red-50 border-red-200' : gap.status === 'overdue' ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-200'}`}>
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <p className="text-xs font-semibold text-gray-900">{gap.activityName}</p>
                                <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${priorityColors[gap.priority]}`}>{gap.priority}</span>
                              </div>
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            ...
                            {gap.estimatedRevenue && (
                              <span className="text-xs text-emerald-700">${gap.estimatedRevenue} · {gap.revenueCode}</span>
                            )}
                          ...
                          {proto.activities.map((act) => (
                            <div key={act.id} className="flex items-start gap-2 p-2 bg-gray-50 rounded-lg text-xs">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-gray-500">{frequencyLabel[act.frequency]} · {act.assignedTeam.toUpperCase()}</p>
                                {act.billable && act.estimatedRevenue && (
                                  <p className="text-emerald-700">${act.estimatedRevenue} · {act.revenueCode}</p>
                                )}
                              </div>
                            </div>
                          ))}
