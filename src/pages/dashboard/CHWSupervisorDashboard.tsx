...
export function CHWSupervisorDashboard() {
  ...
  const activeNow = (encounters ?? []).filter(
  ...
  const completedToday = (encounters ?? []).filter(
    (e) =>
      e.status === "completed" &&
      new Date(e.scheduledDate).toDateString() === today,
  );
  const openEscalations = (encounters ?? []).filter(
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={Clock}
          color={overdueCheckouts.length > 0 ? "red" : "blue"}
        />
        <StatCard
        ...
        <StatCard
          ...
          icon={AlertTriangle}
          color={openEscalations.length > 0 ? "orange" : "blue"}
        />
      </div>
                ...
                >
                  <div>
                    <p className="text-sm font-medium text-gray-900">
                      {getPatientName(enc.patientId)}
                    </p>
                    <p className="text-xs text-gray-500">
                      {enc.chwName} · Check-in:{" "}
                      {enc.checkInTime ?? "Not checked in"}
                    </p>
                    {enc.flags.length > 0 && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        {format(parseISO(enc.scheduledDate), "MMM d, yyyy")}
                      </p>
                    </div>
                    <button
