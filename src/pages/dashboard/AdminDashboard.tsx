...
function buildGapTrendChart(gaps: { dueDate: string; status: string }[]) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const overdue = gaps
      .filter((g) => g.status === "overdue" || g.status === "critical")
      .filter((g) => g.dueDate <= dateStr).length;
    const upcoming = gaps.filter((g) => g.status === "upcoming").length;
    days.push({
      label,
      overdue: Math.max(0, overdue - i),
      upcoming: Math.max(0, upcoming - Math.floor(i / 2)),
    });
  }
  return days;
}
...
export function AdminDashboard() {
  ...
  const consentIssues = (patients ?? []).filter(
    (p) => p.consentStatus !== "active",
  ).length;
  const activeStage = patientJourneys.filter(
    (j) => j.stage === "active_care",
  ).length;
  const maintenanceStage = patientJourneys.filter(
    (j) => j.stage === "maintenance",
  ).length;

  // Care gap metrics
  const openGaps = gaps ?? [];
  const criticalGaps = openGaps.filter(
    (g) => g.priority === "stat" || g.priority === "urgent",
  );
  const revenueAtRisk = openGaps
    .filter((g) => g.estimatedRevenue > 0)
    .reduce((s, g) => s + (g.estimatedRevenue ?? 0), 0);
  const avgAdherence =
    patientJourneys.length > 0
      ? Math.round(
          patientJourneys.reduce((s, j) => s + j.adherenceScore, 0) /
  ...
  const missedApts = (appointments ?? []).filter(
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={highRisk}
          icon={AlertTriangle}
          color={highRisk > 5 ? "red" : "orange"}
        />
        <StatCard
          label="Consent Issues"
          value={consentIssues}
          icon={FileCheck}
          color={consentIssues > 0 ? "amber" : "green"}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={AlertTriangle}
          color={criticalGaps.length > 5 ? "red" : "orange"}
        />
        <StatCard
        ...
        <StatCard
          ...
          icon={TrendingUp}
          color={
            avgAdherence >= 75 ? "purple" : avgAdherence >= 55 ? "amber" : "red"
          }
        />
      </div>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                    {j.patient!.firstName[0]}
                    {j.patient!.lastName[0]}
                  </div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="text-right">
                    <p
                      className={`text-sm font-bold ${j.adherenceScore < 40 ? "text-red-600" : j.adherenceScore < 60 ? "text-orange-600" : "text-amber-600"}`}
                    >
                      {j.adherenceScore}%
                    </p>
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${riskColors[j.patient!.riskLevel]}`}
                  >
                    {j.patient!.riskLevel}
                  </span>
                </div>
                  ...
                  .map((gap) => (
                    <tr key={gap.id} className="hover:bg-gray-50">
                      <td className="py-2.5 pr-4">
                        <button
                          onClick={() =>
                            navigate(`/patients/${gap.patient.id}`)
                          }
                      ...
                      <td className="py-2.5 pr-4">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ${teamColors[gap.assignedTeamType]}`}
                        >
                          {gap.assignedTeamType.toUpperCase()}
                        </span>
                      </td>
              ...
              return (
                ...
                >
                  <span className="font-medium text-gray-800">
                    {p ? `${p.firstName} ${p.lastName}` : apt.patientId}
                  </span>
          ...
          {consentIssues === 0 ? (
            ...
            <div className="space-y-2">
              {(patients ?? [])
                ...
                .map((p) => {
                  const journey = patientJourneys.find(
                    (j) => j.patientId === p.id,
                  );
                  return (
                    <div
                      key={p.id}
                      className={`flex items-center justify-between p-3 rounded-lg border ${p.consentStatus === "missing" ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <UserX
                          className={`h-4 w-4 flex-shrink-0 ${p.consentStatus === "missing" ? "text-red-500" : "text-amber-500"}`}
                        />
                        <div className="min-w-0">
                          <button
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${p.consentStatus === "missing" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                      >
                        {p.consentStatus}
                      </span>
                    </div>
