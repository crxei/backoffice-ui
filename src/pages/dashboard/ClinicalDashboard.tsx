...
const cdsAlerts = [
  {
    ...
    severity: "high",
    message:
      "Potential interaction between metformin and lisinopril. Monitor renal function.",
    acknowledged: false,
  },
  {
    id: "CDS-002",
    ...
    severity: "medium",
    message:
      "BMP overdue by 14 days for CHF patient. Electrolyte imbalance risk.",
    acknowledged: false,
  },
  {
    id: "CDS-003",
    ...
    severity: "high",
    message:
      "Patient flagged for 30-day readmission risk score 78%. Intervention recommended.",
    acknowledged: false,
  },
  {
    id: "CDS-004",
    ...
    severity: "low",
    message:
      "ACE inhibitor not prescribed for heart failure patient per ACC/AHA guidelines.",
    acknowledged: true,
  },
];
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        />
        <StatCard
          label="Active CDS Alerts"
          value={activeCDSAlerts.length}
          icon={Zap}
          color={activeCDSAlerts.length > 0 ? "red" : "green"}
        />
        <StatCard
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="font-semibold text-gray-900 mb-4">
          Care Plans Awaiting My Approval
        </h3>
        {pendingApprovals.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-6">
            No care plans pending approval
          </p>
        ) : (
                ...
                {pendingApprovals.map((cp) => (
                  <tr key={cp.id}>
                    <td className="py-3 pr-4 text-sm font-medium text-gray-900">
                      {getPatientName(cp.patient.id)}
                    </td>
                    <td className="py-3 pr-4 text-sm text-gray-700">
                      {cp.title}
                    </td>
                    <td className="py-3 pr-4 text-sm text-gray-500">
                      {format(parseISO(cp.createdDate), "MMM d")}
                    </td>
                    <td className="py-3 pr-4 text-sm text-gray-500">
                      {cp.goalsProgress.total} goals
                    </td>
                    <td className="py-3">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div className="space-y-3">
          {cdsAlerts.map((alert) => (
            <div
              key={alert.id}
              className={`flex items-start justify-between p-4 rounded-lg border ${alert.acknowledged ? "opacity-50 bg-gray-50" : "bg-white border-gray-200"}`}
            >
              <div className="flex items-start gap-3">
                <span
                  className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${severityStyle[alert.severity]}`}
                >
                  {alert.severity}
                </span>
                <div>
                  <p className="text-sm font-medium text-gray-900">
                    {alert.type} — {getPatientName(alert.patientId)}
                  </p>
