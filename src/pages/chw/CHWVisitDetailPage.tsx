...
export function CHWVisitDetailPage() {
  ...
  if (!enc)
    return (
      <div className="text-center py-20 text-gray-500">Encounter not found</div>
    );
  ...
  return (
    <div>
      <button
        onClick={() => navigate("/chw-visits")}
        className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-4"
      >
        <ArrowLeft className="h-4 w-4" /> Back to CSW Visits
      </button>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 max-w-2xl">
        ...
        <dl className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <dt className="text-gray-500 text-xs mb-0.5">Patient</dt>
          ...
          <div>
            <dt className="text-gray-500 text-xs mb-0.5">CSW</dt>
            <dd>{enc.chwName}</dd>
          </div>
          <div>
            <dt className="text-gray-500 text-xs mb-0.5">Scheduled Date</dt>
            <dd>{format(parseISO(enc.scheduledDate), "MMM d, yyyy")}</dd>
          </div>
          <div>
            <dt className="text-gray-500 text-xs mb-0.5">Visit Type</dt>
            <dd className="capitalize">{enc.visitType.replace(/_/g, " ")}</dd>
          </div>
          <div>
            <dt className="text-gray-500 text-xs mb-0.5">Check-In</dt>
            <dd>{enc.checkInTime ?? "Not checked in"}</dd>
          </div>
          <div>
            <dt className="text-gray-500 text-xs mb-0.5">Check-Out</dt>
            <dd>{enc.checkOutTime ?? "Not checked out"}</dd>
          </div>
