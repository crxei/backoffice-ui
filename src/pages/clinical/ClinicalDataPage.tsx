...
const observationsByPatient: Record<
  string,
  Array<{ date: string; type: string; value: string }>
> = {
  "P-001": [
...
export function ClinicalDataPage() {
  ...
  const patientCPs = (carePlans ?? []).filter(
    (cp) => cp.patient.id === selectedPatientId,
  );
  ...
  const columns: Column<Record<string, unknown>>[] = [
    ...
    },
    {
      ...
      header: "Risk",
      render: (r) => (
        <StatusBadge status={(r as unknown as Patient).riskLevel} />
      ),
    },
    {
      key: "conditions",
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Clinical Data"
        description="Patient observations, medications, and conditions"
      />
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">
              Clinical Record — {selectedPatient.firstName}{" "}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedPatient.conditions.map((c) => (
                  <span
                    key={c}
                    className="px-3 py-1.5 bg-blue-50 text-blue-800 text-sm font-medium rounded-full"
                  >
                    {c}
                  </span>
                ))}
