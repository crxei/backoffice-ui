...
export function CareGapsPage() {
  ...
  const critical = open.filter(
    (g) => g.status === "critical" || g.status === "overdue",
  );
  const revenueAtRisk = open
    .filter((g) => g.revenueOpportunity)
    .reduce((sum, g) => sum + (g.estimatedRevenue ?? 0), 0);
  ...
  const filtered = open.filter((g) => {
    ...
    if (
      ...
      !g.activityName.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  const handleResolve = async (gap: CareGap) => {
    try {
      ...
      toast(
        "success",
        "Gap resolved",
        `${gap.activityName} marked as completed`,
      );
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={AlertTriangle}
          color={critical.length > 5 ? "red" : "orange"}
        />
        <StatCard
      ...
      <div className="flex gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <select
                value={teamFilter}
                onChange={(e) => {
                  setTeamFilter(e.target.value as typeof teamFilter);
                  resetPage();
                }}
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {paginated.map((gap) => {
                      const overdueDays = differenceInDays(
                        new Date(),
                        parseISO(gap.dueDate),
                      );
                      return (
                        <tr
                          key={gap.id}
                          className={`${rowHighlight[gap.status] ?? ""} hover:bg-gray-50 transition-colors`}
                        >
                          {/* Priority */}
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              <span
                                className={`h-2 w-2 rounded-full flex-shrink-0 ${priorityDot[gap.priority]}`}
                              />
                          ...
                          <td className="px-4 py-3">
                            <button
                              onClick={() =>
                                navigate(`/patients/${gap.patientId}`)
                              }
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              <span className="text-base leading-none">
                                {activityIcons[gap.activityType]}
                              </span>
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${teamColors[gap.assignedTeam]}`}
                            >
                          ...
                          <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">
                            {gap.lastCompletedDate
                              ? format(
                                  parseISO(gap.lastCompletedDate),
                                  "MMM d, yyyy",
                                )
                              : "—"}
                          </td>
            ...
            {sorted.length > 0 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 text-sm">
                <span className="text-xs text-gray-500">
                  Showing{" "}
                  {Math.min((safePage - 1) * PAGE_SIZE + 1, sorted.length)}–
        ...
        <div className="w-56 flex-shrink-0 space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            {(Object.entries(byTeam) as [AssignedTeam, CareGap[]][]).map(
              ([team, teamGaps]) => {
                const teamCritical = teamGaps.filter(
                  (g) => g.status === "critical" || g.status === "overdue",
                ).length;
                return (
                  <div key={team} className="mb-3 last:mb-0">
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded-full ${teamColors[team]}`}
                      >
                        {teamLabels[team]}
                      </span>
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-1.5">
                      <div
                        ...
                        style={{
                          width: `${Math.min((teamGaps.length / Math.max(open.length, 1)) * 100, 100)}%`,
                        }}
                      />
                    </div>
                    {teamCritical > 0 && (
                      <p className="text-xs text-red-600 mt-0.5">
                        {teamCritical} overdue/critical
                      </p>
                    )}
                  </div>
                );
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="space-y-2">
              {open
                .filter(
                  (g) =>
                    g.revenueOpportunity &&
                    (g.status === "overdue" || g.status === "critical"),
                )
                .sort(
                  (a, b) =>
