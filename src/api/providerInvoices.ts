...
import { middlewareGet, middlewarePost, middlewareDownload } from './middlewareClient'
...
export async function fetchInvoicePreview(params: InvoiceParams): Promise<ProviderInvoicePreview> {
  return middlewareGet<ProviderInvoicePreview>(`/api/provider-invoices/preview?${invoiceQuery(params)}`)
}
...
export async function downloadInvoiceCsv(params: InvoiceParams): Promise<Blob> {
  return middlewareDownload(`/api/provider-invoices/export.csv?${invoiceQuery(params)}`)
}
